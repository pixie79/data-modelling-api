...
impl ODCSParser {
    ...
    /// Parse ODCS/ODCL YAML content and create Table.
    ///
    ...
    pub fn parse(&mut self, yaml_content: &str) -> Result<(Table, Vec<ParserError>)> {
    ...
    fn is_liquibase_format(&self, data: &JsonValue) -> bool {
        if let Some(_) = data.get("databaseChangeLog") {
            return true;
        }
    ...
    fn parse_column(&self, col_data: &JsonValue) -> Result<Column> {
        ...
        if !nullable {
            let has_not_null = column_quality_rules.iter().any(|rule| {
                rule.get("type")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_lowercase().contains("not_null") || s.to_lowercase().contains("notnull"))
                    .unwrap_or(false)
            });
            if !has_not_null {
                let mut not_null_rule = HashMap::new();
                not_null_rule.insert("type".to_string(), serde_json::json!("not_null"));
                not_null_rule.insert("description".to_string(), serde_json::json!("Column must not be null"));
    ...
    fn parse_odcl_v3(&self, data: &JsonValue) -> Result<(Table, Vec<ParserError>)> {
        ...
        if let Some(links_val) = data.get("links") {
            odcl_metadata.insert(
                "links".to_string(),
                json_value_to_serde_value(links_val),
            );
        }
    ...
    fn parse_data_contract_field(
        ...
        let extract_quality_from_obj = |obj: &serde_json::Map<String, JsonValue>| -> Vec<HashMap<String, serde_json::Value>> {
            let mut quality_rules = Vec::new();
            ...
            if let Some(items_val) = items {
                if let Some(items_obj) = items_val.as_object() {
                        ...
                        || items_obj.get("type").and_then(|v| v.as_str()) == Some("object")
                    {
                        ...
                        // Extract nested fields from items.fields if present
                        if let Some(fields_obj) = items_obj.get("fields").and_then(|v| v.as_object()) {
                            for (nested_field_name, nested_field_data) in fields_obj {
                                if let Some(nested_field_obj) = nested_field_data.as_object() {
                                    ...
                                    match self.parse_data_contract_field(&nested_col_name, nested_field_obj, data) {
                                        Ok(mut nested_cols) => {
        ...
        // Check if this is a nested object with fields
